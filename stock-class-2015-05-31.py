...
class stock:
    ...
    def querystock(self):
        ...
        #self.stock['total']=stock['now']*self.number
        return
    ...
    def display(self):
        ...
        if self.stock['now']==self.stock['max']:
            ...
            if self.prev<self.stock['now']:
                ...
                if self.prev==self.stock['now']:
                    self.flag=no*self.times
                    self.fc='White'
                    self.bk='Black'
                else:
                    self.flag=down*self.times
                    self.fc='Green'
                    self.bk='Black'
        ...
        self.result_string=self.result_string+"{:>6.2f} {:5.2f}%".format(self.stock['now'],self.stock['range'])
    ...
    def calc_days(self):
        ...
        #print 'ffff',self.stock['name'],self.stock['hold_days']
        return
...
def OnStock(event):
    ...
    text[repeat-1].Label=pp+"%s=%7.0f %6.2f%% %s=%10.2f"%('收益'.decode('utf-8').encode('gbk'),total_profit,souyilv,'总市值'.decode('utf-8').encode('gbk'),total_hold)
...
app=wx.App()
mytitle=u"'JJB' stock analisy system V1.02 "+u'(2015-05-31)'
w_weight=800
w_height=120
s_height=20
