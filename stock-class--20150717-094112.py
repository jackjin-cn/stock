...
class stock:
    ...
    def display(self):
        ...
        self.result_string=self.result_string+"{:>6.2f}({:<5.2f} {:<5.2f} {:<5.2f}) {:5.2f} {:5.2f}%".format(self.stock['now'],self.stock['max'],self.stock['min'],self.stock['buy'],self.stock['range'],self.stock['range_percent'])
...
app=wx.App()
mytitle=u"'JJB' stock analisy system V1.07 "+u'(2015-07-17)'
modify=u"修改显示，增加了买入价格的显示"
w_weight=1000
w_height=150
s_height=20
#frame = wx.Frame(parent=None,title=mytitle,size=(w_weight,w_height),style=wx.DEFAULT_FRAME_STYLE|wx.STAY_ON_TOP)
frame = wx.Frame(parent=None,title=mytitle,size=(w_weight,w_height),style=wx.DEFAULT_FRAME_STYLE)
other=2
text=['']*(repeat+2)
