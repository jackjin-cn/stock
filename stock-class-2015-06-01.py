...
class stock:
    ...
    def display(self):
        ...
        if self.stock['now']==self.stock['max']:
            ...
            if self.prev<self.stock['now']:
                ...
                if self.prev==self.stock['now']:
                    ...
                    self.flag=self.down*self.times
                    self.fc='Green'
                    self.bk='Black'
        ...
        self.result_string=self.result_string+"{:>6.2f} {:5.2f} {:5.2f}%".format(self.stock['now'],self.stock['range'],self.stock['range_percent'])
...
app=wx.App()
mytitle=u"'JJB' stock analisy system V1.03 "+u'(2015-06-01)'
w_weight=850
w_height=120
s_height=20
